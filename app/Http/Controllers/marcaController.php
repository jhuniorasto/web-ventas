...
class marcaController extends Controller
{
    ...
    {
        $message = '';
        // Aquí se usa find() para obtener un solo modelo, no una colección
        $marca = Marca::find($id);  // Devolverá un solo modelo, no una colección

        if ($marca) {
            // Acceder al estado de la característica de la marca
            if ($marca->caracteristica->estado == 1) {
                // Actualizamos el estado de la característica a 0 (eliminar)
                Caracteristica::where('id', $marca->caracteristica->id)
                    ->update([
                        'estado' => 0
                    ]);
                ...
                // Restauramos la característica cambiando el estado a 1
                Caracteristica::where('id', $marca->caracteristica->id)
                    ->update([
                        'estado' => 1
                    ]);
            ...
            // Si no se encuentra la marca, puedes manejar el error
